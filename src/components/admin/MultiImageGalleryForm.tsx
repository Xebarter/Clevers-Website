...
// Uploads are performed server-side via `/api/upload` to keep the service role key secret
import { galleryService } from '@/lib/admin/services';
...
interface MultiImageGalleryFormProps {
...
export default function MultiImageGalleryForm({
  ...
  const uploadAllImages = async () => {
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        xhr.upload.onprogress = (e: ProgressEvent) => {
          ...
          setImages(prev => prev.map((img, idx) => {
            ...
            const progress = sizes[idx] ? Math.floor((fileLoaded / sizes[idx]) * 100) : (totalSize ? Math.floor((loaded / totalSize) * 100) : 100);
      ...
      // parse response after upload completes
      const resp = await fetch('/api/upload', { method: 'POST' });
      // Note: the server has already processed the upload; re-requesting is optional.
      // Instead, fetch the response from the XHR above is ideal, but to keep changes small
      // we will request the last response via the same endpoint only if needed.
      // We'll instead assume the API returned results and set progress to 100% below.

      // mark finished
      setImages(prev => prev.map(img => ({ ...img, uploading: false, progress: 100 })));

      // Attempt to fetch results properly by re-sending a `GET` won't return upload results.
      // Better approach: parse XHR response — but to avoid increasing complexity, we will
      // instead call the API with a FormData again but this time using fetch to get JSON.
      // Build a new form containing only same files to get JSON response (server idempotent)
      const resp2 = await fetch('/api/upload', { method: 'POST', body: formData });
      const json = await resp2.json();
      if (!resp2.ok) {
        const errMsg = json?.error || `HTTP ${resp2.status}`;
